...
def _count_to_date_range(count: int) -> tuple:
    """
    将 count（获取最近N个交易日）转换为 start_date 和 end_date。

    A股每年约250个交易日，每周5个。
    估算：count 个交易日 ≈ count * 1.4 个自然日。
    """
    end_date = datetime.now().strftime('%Y%m%d')
    # 加5天 buffer 覆盖周末
    start_date = (datetime.now() - timedelta(days=int(count * 1.5) + 5)).strftime('%Y%m%d')
    return start_date, end_date


def _get_hist(symbol: str, count: int, **kwargs):
    """
    获取股票历史数据，兼容新版 akshare（移除了 count 参数）。

    新版 akshare 使用 start_date/end_date 替代 count。
    """
    start_date, end_date = _count_to_date_range(count)
